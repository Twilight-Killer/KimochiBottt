...
def get_id(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        if msg.reply_to_message and msg.reply_to_message.forward_from:
            ...
            msg.reply_text(
                f"{html.escape(user.first_name)} ɪᴅ ᴋᴀᴍᴜ <code>{user.id}</code>.",
                parse_mode=ParseMode.HTML,
            )
        ...
        if chat.type == "private":
            msg.reply_text(
                f"ᴜꜱᴇʀ ɪᴅ ᴍᴜ <code>{chat.id}</code>.", parse_mode=ParseMode.HTML
            )

        else:
            msg.reply_text(
                f"ɢʀᴏᴜᴩ ɪᴅ <code>{chat.id}</code>.", parse_mode=ParseMode.HTML
            )
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>ᴍᴇɴɢᴀᴍʙɪʟ ɪɴꜰᴏʀᴍᴀꜱɪ...</code>", parse_mode=ParseMode.HTML)

    text = (
        f"ㅤ ㅤㅤ    ✦ ᴜsᴇʀ ɪɴғᴏ ✦\n•❅─────✧❅✦❅✧─────❅•\n"
        f"➻ <b>ᴜsᴇʀ ɪᴅ:</b> <code>{user.id}</code>\n"
        f"➻ <b>ɴᴀᴍᴀ ᴅᴇᴘᴀɴ:</b> {html.escape(user.first_name)}"
    )

    if user.last_name:
        text += f"\n➻ <b>ɴᴀᴍᴀ ʙᴇʟᴀᴋᴀɴɢ:</b> {html.escape(user.last_name)}"
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n\n<b>ᴋᴇꜱᴇʜᴀᴛᴀɴ:</b> <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n[<i>{make_bar(int(userhp['percentage']))} </i>{userhp['percentage']}%]"
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nᴛʜɪs ᴜsᴇʀ ɪs ᴀ ᴍᴇᴍʙᴇʀ ᴏғ <b>ᴋɪᴍᴏᴄʜɪ</b>.\n"
        disaster_level_present = True
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/haotogel_result/12012">ᴋʟɪᴋ ᴅɪꜱɪɴɪ.</a>]'.format(
            bot.username
        )
            ...
            message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url=f"https://t.me/haotogel_result/12012"
                            ),
                            InlineKeyboardButton(
                                "Dɪsᴀsᴛᴇʀ", url="https://t.me/haotogel_result/12012"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
            ...
            message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url="https://t.me/haotogel_result/12012"
                            ),
                            InlineKeyboardButton(
                                "ᴅɪsᴀsᴛᴇʀ", url="https://t.me/haotogel_result/12012"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "ᴜᴍᴍ... ʏᴇᴀʜ, ɪ ᴏɴʟʏ ᴛʀᴜsᴛ ᴀssᴏᴄɪᴀᴛɪᴏᴍ ᴛᴏ sᴇᴛ ᴍʏ ʙɪᴏ."
            )
            return
